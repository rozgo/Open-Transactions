...
int OTPaymentPlan::ProcessXMLNode(irr::io::IrrXMLReader*& xml)
{
	...
	if (!strcmp("paymentPlan", xml->getNodeName())) 
	{
...
void OTPaymentPlan::UpdateContents()
{
	...
	{
		OTASCIIArmor ascConsideration(m_strConsideration);		
		m_xmlUnsigned.Concatenate("<consideration>\n%s</consideration>\n\n", ascConsideration.Get());
	}
...
OTPaymentPlan::OTPaymentPlan() : OTAgreement()
{
	// These are only used during execution, and are not stored:
	m_bProcessingInitialPayment	= false;
	m_bProcessingPaymentPlan	= false;
	m_bFlagForRemoval			= false;
...
OTPaymentPlan::OTPaymentPlan(const OTIdentifier & SERVER_ID,			const OTIdentifier & ASSET_ID,
					...
					OTAgreement(SERVER_ID, ASSET_ID, SENDER_ACCT_ID, SENDER_USER_ID, RECIPIENT_ACCT_ID, RECIPIENT_USER_ID)
{
	// These are only used during execution, and are not stored:
	m_bProcessingInitialPayment	= false;
	m_bProcessingPaymentPlan	= false;
...
{	
	...
	if (SOURCE_ACCT_ID == RECIPIENT_ACCT_ID)
	{
		...
		SetFlagForRemoval(); // Remove from Cron
	...
	pOrigPlan		= OTCronItem::LoadCronReceipt(GetTransactionNum());
	
	OT_ASSERT(NULL != pOrigPlan);	// How am I processing it now if the receipt wasn't saved in the first place??
									// TODO: Decide global policy for handling situations where the hard drive stops working, etc.
	
	// When theOrigPlanGuardian goes out of scope, pOrigPlan gets deleted automatically.
	OTCleanup<OTCronItem>	theOrigPlanGuardian(*pOrigPlan);
		
	// strOrigPlan is a String copy (a PGP-signed XML file, in string form) of the original Payment Plan request...
	OTString strOrigPlan(*pOrigPlan); // <====== Farther down in the code, I attach this string to the receipts.
	
	
	// Make sure to clean these up.
//	delete pOrigPlan;		// theOrigPlanGuardian will handle this now, whenever it goes out of scope.
//	pOrigPlan = NULL;		// So I don't need to worry about deleting this anymore. I can keep it around and
	...
	else	// Else load the First Nym from storage.
	{
		...
		if (false == theSenderNym.LoadPublicKey())
		{
			...
			OTLog::vError("Failure loading Sender Nym public key in OTPaymentPlan::ProcessPayment: %s\n", 
						  strNymID.Get());
			SetFlagForRemoval(); // Remove it from future Cron processing, please.
			return false;
		}
	...
	if (!pOrigPlan->VerifySignature(*pSenderNym) || !pOrigPlan->VerifySignature(*pRecipientNym))
	{
		OTLog::Error("Failed authorization: Payment plan (while attempting to process...)\n");
		SetFlagForRemoval(); // Remove it from Cron.
		return false;			
	}
	...
	//
	// I also have pOrigPlan, which is a dynamically-allocated copy of the original
	// Cron Receipt for this Payment Plan. (And I don't need to worry about deleting it, either.)
	// I know for a fact they have both signed pOrigPlan...
	
	
	// Make sure have BOTH accounts loaded and checked out. 
	// (sender's asset account and recipient's asset account.)
	
//	OTAccount * pSourceAcct		= NULL;
//	OTAccount * pRecipientAcct	= NULL;

	// No need for OT_ASSERT here, but we do need to check if the accounts even exist:
...
//			OT_ASSERT(lNewTransactionNumber > 0); // this can be my reminder.			
			...
			// ORIGINAL SIGNED PLAN. (With both parties' original signatures on it.)
			//
...
//					   GetProcessInterval());
	...
	// In OTTrade, I couldn't check for this so early, because I had to find the Market
	// pointer first before I could remove the offer from the market, (before I could
	// actually remove the Trade from Cron.)
	// 
	// But here, in OTPaymentPlan, there is neither market nor offer, just this object.
	// So if it's been flagged for Removal, then I can check for that right away,
	// and return, since any code below that would be irrelevant.
...
//			OTLog::Output(3, "DEBUG: Not enough time has elapsed.\n");	
	...
	{
		OTLog::Output(3, "Cron: Removing payment plan from cron processing...\n");
